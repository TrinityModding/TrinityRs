...
fn main() {
    ...
    let _standard = PipelineCreationInfo {
        attributes: vec![
            VertexAttributeInfo { binding: 0, format: Format::R32G32B32_SFLOAT }, // Pos
            VertexAttributeInfo { binding: 1, format: Format::R32G32_SFLOAT }, // TexCoords
        ],
    }.create(&renderer, standard_stages, layout.clone());

    // Graph setup
    let fbo = Rc::new(RwLock::new(WindowFrameBuffer::new(&mut renderer.viewport, renderer.allocator.clone(), info.1.as_slice())));
    let mut graph = SceneGraph::new(fbo.clone(), layout, &renderer);
    graph.add_shader("Standard", _standard.clone(), renderer.allocator.clone());
    graph.add_shader("SSS", _standard.clone(), renderer.allocator.clone());
    graph.add_shader("EyeClearCoat", _standard.clone(), renderer.allocator.clone());
    graph.add_shader("FresnelBlend", _standard.clone(), renderer.allocator.clone());

    // Load model into graph
    let pokemon = "pm1018";
    let _lod_mesh = from_trmdl(format!("A:/PokemonScarlet/pokemon/data/{}/{}_00_00/{}_00_00.trmdl", pokemon, pokemon, pokemon), &mut graph);
    ...
    renderer.previous_frame_end = Some(
        uploads
            .build().unwrap()
            .execute(renderer.graphics_queue.clone()).unwrap()
            .boxed(),
    );
        ...
        match event {
            ...
            Event::RedrawEventsCleared => {
                ...
                if renderer.recreate_swapchain {
                    ...
                    fbo.write().unwrap().update(&mut renderer.viewport, renderer.allocator.clone(), new_images.as_slice());
...
fn read_spirv_words_from_file(path: impl AsRef<Path>) -> Vec<u32> {
    ...
    let mut file = File::open(path).unwrap_or_else(|err| {
        panic!("can't open file `{}`: {}.", path.display(), err)
    });
